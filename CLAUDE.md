...
3. **Content** (`content.py`) - `ContentProcessor` loads source files, extracts metadata (date from filename, tags from #hashtags, title from first `#` heading), renders Markdown via mistune
...
- `current_page` - Current Page object (title, content, url, date, tags, toc, excerpt, frontmatter)
- `pages` - PageCollection with `.group()`, `.with_tag()`, `.published()`, `.sorted()`, `.latest(n)`
